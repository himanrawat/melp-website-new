...
// Helper functions
function getStepClassName(step: Step, currentStep: number): string {
...
function getStepCursorClassName(step: Step, currentStep: number): string {
	if (step.id <= currentStep) {
...
function calculatePrice(
	...
	return { unitPrice: basePrice, total: basePrice * formData.numberOfUsers };
}
						...
						>
							<motion.div
								...
								className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold transition-all duration-200 z-10
									${getStepClassName(step, currentStep)}
									${getStepCursorClassName(step, currentStep)}
								`}
							>
								{step.id < currentStep ? (
...
function EnterpriseFeaturesList({
	selectedPlan,
	isFeaturesExpanded,
	setIsFeaturesExpanded,
	maxVisibleFeatures,
	featuresContainerRef,
}: {
	selectedPlan: PricingPlan;
	isFeaturesExpanded: boolean;
	setIsFeaturesExpanded: (value: boolean) => void;
	...
	const featuresToShow = isFeaturesExpanded
		...
		: selectedPlan.features.filter((f) => f.highlight).slice(0, maxVisibleFeatures);
		...
		<div className="flex-1 flex flex-col min-h-0 pt-5 border-t">
			...
			<div className="relative flex-1 flex flex-col min-h-0">
				<div
					...
					style={isFeaturesExpanded ? { maxHeight: "calc(100% - 40px)" } : undefined}
				>
				...
				{!isFeaturesExpanded && (
					<>
						...
						>
							<span>+{selectedPlan.features.length - maxVisibleFeatures} additional capabilities</span>
							<motion.svg
								className="w-3 h-3"
								fill="none"
								viewBox="0 0 24 24"
...
function EnterpriseSidebar({
	...
	selectedPlan: PricingPlan | null;
	enterpriseCardRef: React.RefObject<HTMLDivElement | null>;
	currentBenefitIndex: number;
	onNextBenefit: () => void;
	onPrevBenefit: () => void;
	onSelectBenefit: (index: number) => void;
	isFeaturesExpanded: boolean;
	setIsFeaturesExpanded: (value: boolean) => void;
	maxVisibleFeatures: number;
	featuresContainerRef: React.RefObject<HTMLDivElement | null>;
		...
		>
			<div className="mb-5 shrink-0">
				...
				<p className="text-sm text-muted-foreground leading-relaxed">
					Unlock the full potential of Melp with our most comprehensive plan, designed for
			...
			<BenefitsCarousel
				...
				onNext={onNextBenefit}
				onPrev={onPrevBenefit}
			...
			{selectedPlan && (
				<EnterpriseFeaturesList
					...
					setIsFeaturesExpanded={setIsFeaturesExpanded}
					maxVisibleFeatures={maxVisibleFeatures}
			...
			>
				<p className="text-xs text-foreground font-medium mb-1">Built for Success</p>
				<p className="text-[10px] text-muted-foreground leading-relaxed">
					Join leading enterprises who trust Melp to power their communication, collaboration,
				...
				{selectedPlan && (
					<div className="space-y-3">
						...
						<div className="flex justify-between items-center">
							...
							<span className="text-sm font-medium text-foreground">{formData.numberOfUsers}</span>
						</div>
						...
						<div className="pt-3 mt-3 border-t">
							<div className="flex justify-between items-baseline">
								<div>
									...
									<p className="text-[10px] text-muted-foreground">Tax calculated at checkout</p>
								</div>
								<div className="text-right">
									<span className="text-xl font-bold text-foreground">
										{currency}
										{(total * (formData.subscriptionLength === "1-year" ? 12 : 1)).toFixed(2)}
									</span>
						...
						<div className="flex items-start justify-between gap-2">
							<div className="flex-1">
								<div className="flex items-center gap-1.5 mb-1">
									...
									<p className="text-xs font-semibold text-foreground">Upgrade & save more</p>
								</div>
								<p className="text-[10px] text-muted-foreground leading-relaxed">
									{selectedPlan.id === "free" || selectedPlan.id === "plus"
...
export default function CheckoutLayout({
	...
	useEffect(() => {
		...
		const calculateVisibleFeatures = () => {
			...
			const roiHeight = 80;
			const featureHeaderHeight = 50;
			const buttonHeight = 40;
			const padding = 40;

			const availableHeight =
				cardHeight - headerHeight - carouselHeight - roiHeight - featureHeaderHeight - buttonHeight - padding;

			const featureItemHeight = 28;
			const calculatedMax = Math.floor(availableHeight / featureItemHeight);
			const finalMax = Math.max(3, calculatedMax);

			const highlightedCount = selectedPlan.features.filter((f) => f.highlight).length;
	...
	const prevBenefit = () => {
		setCurrentBenefitIndex((prev) => (prev - 1 + enterpriseBenefits.length) % enterpriseBenefits.length);
	};

	const { unitPrice, total } = calculatePrice(selectedPlan, formData);
			...
			<header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b">
				<div className="flex items-center justify-between px-4 lg:px-8 py-3">
					<Link href="/" className="flex items-center">
						<Image src="/logo.svg" alt="Melp" width={100} height={28} className="dark:hidden" />
						<Image
							src="/logo-dark.svg"
							alt="Melp"
							width={100}
			...
			<main className="pt-14 flex flex-col lg:flex-row min-h-screen">
				<div className="flex-1 lg:pr-[380px] xl:pr-[420px]">
					<div className="max-w-xl mx-auto px-4 lg:px-8 py-6 lg:py-8">
						{!isEnterprise && (
							<ProgressStepper steps={visibleSteps} currentStep={currentStep} onStepClick={onStepClick} />
						)}

						<motion.div key={`step-${currentStep}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-4">
				...
				<div className="hidden lg:block fixed right-0 top-14 bottom-0 w-[380px] xl:w-[420px] border-l bg-muted/30">
					<div className="h-full flex flex-col p-6 xl:p-8">
						{isEnterprise ? (
							<EnterpriseSidebar
								selectedPlan={selectedPlan}
								enterpriseCardRef={enterpriseCardRef}
								currentBenefitIndex={currentBenefitIndex}
								onNextBenefit={nextBenefit}
								onPrevBenefit={prevBenefit}
								onSelectBenefit={setCurrentBenefitIndex}
								isFeaturesExpanded={isFeaturesExpanded}
								setIsFeaturesExpanded={setIsFeaturesExpanded}
								maxVisibleFeatures={maxVisibleFeatures}
								featuresContainerRef={featuresContainerRef}
						...
						<div className="pt-6">
							<div className="flex flex-wrap gap-3 text-[10px] text-muted-foreground justify-end">
								...
								<a href="/legal/terms-conditions" className="hover:text-foreground transition-colors">
									Terms
								</a>
			...
			<div className="lg:hidden fixed bottom-0 left-0 right-0 bg-card border-t p-3 z-40">
				<div className="flex items-center justify-between max-w-xl mx-auto">
					<div>
						...
						{!isEnterprise && (
							<p className="text-xs text-muted-foreground">
								{formData.numberOfUsers} user{formData.numberOfUsers > 1 ? "s" : ""} •{" "}
								{formData.billingFrequency === "yearly" ? "Annual" : "Monthly"}
							</p>
						)}
					</div>
